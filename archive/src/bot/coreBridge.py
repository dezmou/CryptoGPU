...
class Data:
    ...
    @staticmethod
    def getRandomSituation(size, coinList=None):
        if coinList is None:
            coinList = Data.getCoinList()
        coin = coinList[random.randint(0, len(coinList) - 1)]
        fileSize = int(os.stat("./data/{}".format(coin)).st_size / 6 / 8)
        minuteId = random.randint(0, fileSize - 5000)
        return {
            "data": Data.getSituation(size, coin, minuteId),
            "coin": coin,
            "minuteId": minuteId
        }
