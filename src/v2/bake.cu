...
static void bake(Data data) {
    ...
    for (int chien = 0; chien < 100000; chien++) {
        ...
        for (int i = 0; i < nbrWorkers; i++) {
            // printf("BK: %-12.2lf\n\n", brokers[i].bank);
            if (brokers[i].bank > 0 && brokers[i].reg >= maxReg - 1 && brokers[i].nbrBets > BAKE_MIN_BETS) {
            // if (brokers[i].bank > 0 && brokers[i].reg >= maxReg && brokers[i].nbrBets > BAKE_MIN_BETS) {
